...
new Tabular.Table({
  ...
  collection: OfertaMateria,
  columns: [
    {data:"" , title:" ", defaultContent:'<input class="checkBox" type="checkbox">'},
  	{data:"Turma", title:"Turma"},
  	{data:"Materia.nomeMateria", title:"Materia"},
  ],
	extraFields:[
    ...
    settingsTurma: function() {
      ...
      selectTurmaEscolha2:function(){
          ...
          var id=Session.get('turmaSelecionada');
      		return {Processo:processo,_id:{$ne:id._id}}
      	},
    mostrar(){
...
if(Meteor.isServer){
  Meteor.methods({
		'retirar':function(id, array){
			for(x=0;x<array.length;x++){
				OfertaMateria.update({_id:id},{ $pull:{restricao:array[x].toString()}})
				OfertaMateria.update({_id:array[x].toString()},{ $pull:{restricao:id}})
			}
		},
    'restrigir':function(id,array){
      ...
      OfertaMateria.update({_id:id},{$set:{restricao:idArray}})
    },
		'resgtrigirUnico':function(id, res, array){
			//console.log(array);
			OfertaMateria.update({_id:id},{ $addToSet:{restricao:res}})
		}
  })
}
